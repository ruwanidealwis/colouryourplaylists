...
colourGenerator.getPalette(playlistQueried.audioFeatures.valence,playlistQueried.audioFeatures.energy,playlistQueried.audioFeatures.mode,playlistQueried.audioFeatures.danceability).then(
  palette => {
    ...
    let username = spotifyController.username;
    res.json({playListArray:allPlaylistInfo, palette: palette, playlistName: playlistQueried.name, coverImage: playlistQueried.coverUrl, name:username, colourInfo: colourInfo });
...
colourGenerator.getPalette(songQueried.audioFeatures.valence,songQueried.audioFeatures.energy,songQueried.audioFeatures.mode,songQueried.audioFeatures.danceability).then(
  palette => {
    ...
    let colourInfo = colourGenerator.colourInfo;
    console.log(colourInfo);
    res.json({playListArray:allPlaylistInfo, palette: palette, songName: songQueried.name, coverImage:songQueried.coverUrl, artist:songQueried.artist, colourInfo: colourInfo });
    //res.send(palette);
  }
);
});
