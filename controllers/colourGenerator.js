...
let minorcolour = [new colour("oxford blue", [0,0,51]), new colour("black",[31, 29, 25]),new colour("dark green",[0,51,26]),new colour("seal brown",[51,0,9])];
...
let infoAboutColours = function(colours,valence,energy,danceability, mode)
{
  let arrayColourInfo = [];
    ...
    arrayColourInfo[0] = {name:colours[0].__name, danceability:overallIinfo.danceability, energy: overallIinfo.energy};
    arrayColourInfo[1] = {name:colours[1].__name, valence:overallIinfo.valence};
...
function getInitialValues(valence,energy,mode,danceability)
{
  ...
  if(mode==1){
    let modecolour = majorColour[Math.floor(Math.random() * majorColour.length)];
    let colourToAdd = JSON.parse(JSON.stringify(modecolour));
    overallIinfo.mode="major";
     colours.push(colourToAdd);
  }
  else {
    let modecolour = minorcolour[Math.floor(Math.random() * minorcolour.length)];
    let colourToAdd = JSON.parse(JSON.stringify(modecolour))
    colours.push(colourToAdd);
